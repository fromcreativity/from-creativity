 ...
 *  Server URL: https://cdn.jsdelivr.net/gh/fromcreativity/from-creativity@main/from-creativity-element-20260813d.js
 *
...
`;
  ...
  class FromCreativityPageV4 extends HTMLElement {
    connectedCallback() {
      ...
      /* 高さ調整：描画・画像読込・リサイズのタイミングで実行 */
      [600, 1800, 3500, 6000].forEach(t => setTimeout(() => this.syncWixHeight(), t));
      window.addEventListener('load', () => this.syncWixHeight());
      let rt = null;
      window.addEventListener('resize', () => {
        clearTimeout(rt);
        rt = setTimeout(() => this.syncWixHeight(), 300);
      }, { passive: true });
    }
    ...
    /* Wixウィジェットの高さが中身より高いと、下に黒い余白が残る。
       実際のコンテンツ高さを測ってラッパーを詰める。
       低くなる方向にだけ効かせ、レイアウトを壊さないようにする。 */
    syncWixHeight() {
      try {
        const app = this.querySelector('.fc-app');
        if (!app) return;
        const contentH = Math.ceil(app.getBoundingClientRect().height);
        if (!contentH || contentH < 400) return;   // 未レンダリング時は触らない
        let el = this.parentElement;
        let steps = 0;
        while (el && el !== document.body && el !== document.documentElement && steps < 4) {
          const h = Math.ceil(el.getBoundingClientRect().height);
          if (h - contentH > 40) {                 // 明らかに余っているときだけ
