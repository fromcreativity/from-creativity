 ...
 *  Server URL: https://cdn.jsdelivr.net/gh/fromcreativity/from-creativity@main/from-creativity-element-20260813g.js
 *
...
@media (max-width:768px){
  ...
  .fc-app .hero{padding-left:0 !important;padding-right:0 !important;padding-top:76px !important;padding-bottom:56px !important}
...
@media (max-width:480px){
  ...
  .fc-app .hero{padding-left:0 !important;padding-right:0 !important;padding-top:68px !important;padding-bottom:48px !important}
}
...
`;
  ...
  class FromCreativityPageV4 extends HTMLElement {
    connectedCallback() {
      ...
      [800, 2000, 4000, 7000].forEach(t => setTimeout(() => this.trimWixGap(), t));
      window.addEventListener('load', () => setTimeout(() => this.trimWixGap(), 500));
      let _rt = null;
    ...
    /* Wixウィジェットの高さが中身より大きいと、下に黒い空白が残る。
       実測して、余っている祖先の高さを詰める。縮める方向にのみ作用。 */
    trimWixGap() {
      try {
        const app = this.querySelector('.fc-app');
        if (!app) return;
        const contentH = Math.ceil(app.scrollHeight);
        if (!contentH || contentH < 500) return;
        if (this._lastTrim === contentH) {
          // 高さが変わっていなければ祖先だけ点検する
        } else {
          this._lastTrim = contentH;
        }
        // ホスト自身
        this.style.setProperty('height', contentH + 'px', 'important');
        ...
        while (el && el !== document.body && el !== document.documentElement && n < 5) {
        ...
        // Wix が高さを再設定してきたら検知して詰め直す
        if (!this._ro && 'ResizeObserver' in window && this.parentElement) {
          this._ro = new ResizeObserver(() => {
            clearTimeout(this._roT);
            this._roT = setTimeout(() => {
              const c = Math.ceil(app.scrollHeight);
              const w = Math.ceil(this.parentElement.getBoundingClientRect().height);
              if (c >= 500 && w - c > 30) this.trimWixGap();
            }, 250);
          });
