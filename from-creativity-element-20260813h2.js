 ...
 *  Server URL: https://cdn.jsdelivr.net/gh/fromcreativity/from-creativity@main/from-creativity-element-20260813h2.js
 *
...
`;
  ...
  class FromCreativityPageV4 extends HTMLElement {
    connectedCallback() {
      ...
      [1200, 2500, 4500, 7000, 10000].forEach(t => setTimeout(() => this.trimWixGap(), t));
    ...
    trimWixGap() {
      try {
        ...
        const last = this.querySelector('.tagline');
        if (!app || !last) return;
        const appTop = app.getBoundingClientRect().top;
        const realH  = Math.ceil(last.getBoundingClientRect().bottom - appTop) + 2;
        if (!realH || realH < 500) return;          // 未レンダリング時は触らない

        app.style.setProperty('height', realH + 'px', 'important');
        app.style.setProperty('min-height', '0', 'important');
        app.style.setProperty('max-height', realH + 'px', 'important');
        ...
        if (!this._ro && 'ResizeObserver' in window) {
          this._ro = new ResizeObserver(() => {
            ...
            this._roT = setTimeout(() => this.trimWixGap(), 300);
          });
