 ...
 *  Server URL: https://cdn.jsdelivr.net/gh/fromcreativity/from-creativity@main/from-creativity-element-20260813i.js
 *
...
(function(){
  ...
  const VERSION = '2026-08-13i-MINHEIGHT';
...
`;
  ...
  class FromCreativityPageV4 extends HTMLElement {
    connectedCallback() {
      ...
      [2000, 5000, 9000].forEach(t => setTimeout(() => this.fcDebugPanel(), t));
      let _rt = null;
    ...
    trimWixGap() {
      try {
        ...
        /* ── ① <main> を必ず「中身の高さ」に戻す ──────────────
           min-height は max-height / height より強いので、
        ...
        /* ── ② ホストと祖先を実測値でクランプ ────────────── */
        const clamp = (el) => {
          el.style.setProperty('min-height', '0', 'important');   // ← 最重要
        ...
        while (el && el !== document.body && el !== document.documentElement && n < 6) {
          ...
          this._chain.push({
            ...
            h: h, minH: cs.minHeight, disp: cs.display, over: h - realH
          });
          if (h - realH > 8) clamp(el);
          el = el.parentElement; n++;
        }

        /* ── ③ Wix が style を書き戻したら再適用（暴走防止付き） ── */
        if (!this._mo) {
          this._reapply = () => {
            if (this._busy) return;
            this._busy = true;
            clearTimeout(this._moT);
            this._moT = setTimeout(() => { this._busy = false; this.trimWixGap(); }, 250);
          };
          this._mo = new MutationObserver(this._reapply);
          let p = this.parentElement, m = 0;
          this._mo.observe(this, { attributes: true, attributeFilter: ['style'] });
          while (p && p !== document.body && m < 6) {
            this._mo.observe(p, { attributes: true, attributeFilter: ['style'] });
            p = p.parentElement; m++;
          }
        }
