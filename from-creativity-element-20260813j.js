 ...
 *  Server URL: https://cdn.jsdelivr.net/gh/fromcreativity/from-creativity@main/from-creativity-element-20260813j.js
 *
...
(function(){
  ...
  const VERSION = '2026-08-13j-NEGMARGIN';
...
`;
  ...
  class FromCreativityPageV4 extends HTMLElement {
    ...
    trimWixGap() {
      try {
        if (/[?&]fcsafe=1/.test(location.search)) return;  // 切り分け用セーフモード
        const app = this.querySelector('.fc-app');
        if (!app) return;

        /* ── ① <main> を必ず「中身の高さ」に戻す ──────────────
           CSS では min-height が height / max-height より強い。
           必ず 0 にリセットしてから測る。ここが今までの敗因。 */
        const freeStyle = (el) => {
        ...
        const clamp = (el) => {
          el.style.setProperty('min-height', '0', 'important');
          el.style.setProperty('height', realH + 'px', 'important');
          el.style.setProperty('max-height', realH + 'px', 'important');
        ...
        while (el && el !== document.body && el !== document.documentElement && n < 6) {
          chain.push(el);
          const h = Math.ceil(el.getBoundingClientRect().height);
          if (h - realH > 8) clamp(el);
          el = el.parentElement; n++;
        }

        /* ── ③ それでも高さが残るなら、負のマージンで物理的に詰める ──
           min-height / height が何であろうと、負の margin-bottom は
           必ずレイアウト上の高さを削る。最後の砦。 */
        const pull = (el, keep) => {
          const h = Math.ceil(el.getBoundingClientRect().height);
          const over = h - keep;
          if (over > 8) {
            el.style.setProperty('margin-bottom', '-' + over + 'px', 'important');
            return over;
          }
        ...
        pull(this, realH);
        chain.forEach(a => pull(a, Math.ceil(this.getBoundingClientRect().height)));

        /* 診断用に状態を保存 */
        this._diag = { realH: realH, hostH: Math.ceil(this.getBoundingClientRect().height),
                       chain: chain.map(c => ({
                         tag: c.tagName.toLowerCase(), id: c.id || '-',
        ...
        if (!this._mo) {
          ...
          chain.forEach(c => this._mo.observe(c, { attributes: true, attributeFilter: ['style'] }));
        }
    ...
    /* 診断モード
       起動方法は3通り（どれか1つでOK）
        1) URL に ?fcdebug=1 を付ける
        2) URL の末尾に #fcdebug を付ける
        3) 左上のロゴを5回すばやくタップ            */
    initDebugTrigger() {
      try {
        const on = () => /[?&]fcdebug=1/.test(location.search) || /fcdebug/.test(location.hash);
        if (on()) [600, 1800, 3500, 6000, 10000].forEach(t => setTimeout(() => this.fcDebugPanel(), t));
    ...
    fcDebugPanel() {
      try {
        ...
        if (!box) {
          ...
          box.id = 'fcDebugBox';
          box.style.cssText = 'position:fixed;left:6px;right:6px;bottom:6px;z-index:2147483647;' +
            'background:#fff;color:#111;font:11px/1.5 ui-monospace,monospace;padding:10px 12px;' +
            'border:3px solid #E94B2B;border-radius:8px;max-height:60vh;overflow:auto;' +
        ...
        o += 'host   h=' + Math.round(hostR.height) + ' minH=' + hcs.minHeight +
             ' disp=' + hcs.display + ' mb=' + hcs.marginBottom +
             ' 差=' + Math.round(hostR.height - appR.height) + '\n';
        (d.chain || []).forEach((c, i) => {
          o += '親' + (i + 1) + ' <' + c.tag + '> ' + c.id + '\n  .' + c.cls +
               '\n  h=' + c.h + ' minH=' + c.minH + ' disp=' + c.disp + '\n';
        });
