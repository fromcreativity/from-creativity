 ...
 *  Server URL: https://cdn.jsdelivr.net/gh/fromcreativity/from-creativity@main/from-creativity-element-20260813k.js
 *
...
(function(){
  ...
  const VERSION = '2026-08-13k-CONTENTS';
  ...
  class FromCreativityPageV4 extends HTMLElement {
    ...
    trimWixGap() {
      try {
        ...
        /* ── 段階1｜<main> を中身の高さに戻す ──────────────────
           CSS では min-height が height / max-height より強い。
           必ず 0 にリセットしてから測る。 */
        ...
        const clamp = (e2) => {
          ...
          e2.style.setProperty('height', realH + 'px', 'important');
          e2.style.setProperty('max-height', realH + 'px', 'important');
          e2.style.setProperty('overflow', 'hidden', 'important');
          e2.style.setProperty('align-self', 'start', 'important');
          e2.style.setProperty('align-content', 'start', 'important');
          e2.style.setProperty('flex', '0 0 auto', 'important');
        };
        this.style.setProperty('display', 'block', 'important');
        clamp(this);
        ancestors.forEach(a => {
          if (Math.ceil(a.getBoundingClientRect().height) - realH > 8) clamp(a);
        });

        /* ── 段階2｜負の margin-bottom で物理的に詰める ──────────
           height をどう上書きされても、負のマージンは効く。 */
        const pull = (e2, keep) => {
          const over = Math.ceil(e2.getBoundingClientRect().height) - keep;
          if (over > 8) e2.style.setProperty('margin-bottom', '-' + over + 'px', 'important');
          else e2.style.setProperty('margin-bottom', '0px', 'important');
        };
        pull(this, realH);
        ancestors.forEach(a => pull(a, Math.ceil(this.getBoundingClientRect().height)));

        /* ── 段階3｜display:contents で箱そのものを消す ──────────
           display:contents の要素はボックスを生成しないため、
           height も min-height も一切効かなくなる。最終手段。
           幅が壊れたら即座に元に戻す。 */
        const stillOver = (e2) => Math.ceil(e2.getBoundingClientRect().height) - realH > 40;
        if (stillOver(this)) {
          const w0 = Math.round(app.getBoundingClientRect().width);
          this.style.setProperty('display', 'contents', 'important');
          const w1 = Math.round(app.getBoundingClientRect().width);
          if (w1 < w0 * 0.6 || w1 < 200) this.style.setProperty('display', 'block', 'important');
        }
        for (let a = 0; a < ancestors.length; a++) {
          const node = ancestors[a];
          if (Math.ceil(node.getBoundingClientRect().height) - realH <= 40) break;
          const cs = getComputedStyle(node);
          if (cs.position === 'absolute' || cs.position === 'fixed') continue; // 絶対配置は触らない
          const w0 = Math.round(app.getBoundingClientRect().width);
          const prev = node.style.display;
          node.style.setProperty('display', 'contents', 'important');
          const w1 = Math.round(app.getBoundingClientRect().width);
          if (w1 < w0 * 0.6 || w1 < 200) node.style.display = prev;
        }
        ...
        /* 隙間が残っていたら、URL指定なしで自動的に診断パネルを出す */
        const gap = this._diag.hostH - realH;
        if (gap > 80 && window.innerWidth <= 900) this.fcDebugPanel(true);
    ...
    fcDebugPanel(auto) {
      try {
        ...
        if (!box) {
          ...
          box.id = 'fcDebugBox';
          box.style.cssText = 'position:fixed;left:6px;right:6px;top:6px;z-index:2147483647;' +
            'background:#fff;color:#111;font:11px/1.5 ui-monospace,monospace;padding:10px 12px;' +
        ...
        o += 'viewport ' + window.innerWidth + 'x' + window.innerHeight + '\n';
        o += 'main h=' + Math.round(appR.height) + '\n';
        o += 'host h=' + Math.round(hostR.height) + ' minH=' + hcs.minHeight +
             ' disp=' + hcs.display + ' pos=' + hcs.position +
             ' mb=' + hcs.marginBottom + ' 差=' + Math.round(hostR.height - appR.height) + '\n';
        (d.chain || []).forEach((c, i) => {
          o += '親' + (i + 1) + ' <' + c.tag + '> ' + c.id + '\n  .' + c.cls +
               '\n  h=' + c.h + ' minH=' + c.minH + ' disp=' + c.disp + ' pos=' + c.pos +
               ' 差=' + (c.h - Math.round(appR.height)) + '\n';
        });
        o += 'body h=' + Math.round(document.body.getBoundingClientRect().height) +
             '  docH=' + document.documentElement.scrollHeight;
        if (this._err) o += '\nERR ' + this._err;
        box.textContent = o;
      } catch (e) { /* noop */ }
    ...
    fcDebugPanel() {
      try {
        if (!/[?&]fcdebug=1/.test(location.search)) return;
        const app = this.querySelector('.fc-app');
        if (!app) return;
        let box = document.getElementById('fcDebugBox');
        if (!box) {
          ...
          box.style.cssText = 'position:fixed;left:8px;right:8px;bottom:8px;z-index:2147483647;' +
            'background:#fff;color:#111;font:11px/1.5 monospace;padding:10px 12px;' +
            'border:2px solid #E94B2B;border-radius:8px;max-height:52vh;overflow:auto;' +
        ...
        const hcs   = getComputedStyle(this);
        let out = '■ FC DIAG (' + (VERSION || '') + ')  ※タップで閉じる\n';
        out += 'viewport : ' + window.innerWidth + ' x ' + window.innerHeight + '\n';
        out += 'main     : h=' + Math.round(appR.height) + '\n';
        out += 'host     : h=' + Math.round(hostR.height) +
               '  minH=' + hcs.minHeight + '  disp=' + hcs.display +
               '  差=' + Math.round(hostR.height - appR.height) + '\n';
        (this._chain || []).forEach((c, i) => {
          out += '親' + (i + 1) + ' <' + c.tag + '> id=' + c.id + '\n';
          out += '     class=' + c.cls + '\n';
          out += '     h=' + c.h + ' minH=' + c.minH + ' disp=' + c.disp + ' 差=' + c.over + '\n';
        });
