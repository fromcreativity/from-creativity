...
(function(){
  ...
  const VERSION = '2026-09-03V-CTAFIX';
    ...
    hideCtaOnTagline() {
      try {
        ...
        const update = () => {
          ...
          /* タグライン（最終セクション）が画面に入ってきたら隠す。
             ここには「特定商取引法に基づく表記」のボタンがあり、
             フロートCTAが重なって押せなくなるため。 */
          const reached = rect.top < window.innerHeight - 80;
          setHidden(reached);
        };
    ...
    initInteractions() {
      ...
      if (toggle && menu) {
        ...
        toggle.addEventListener('click', () => {
          ...
          const ctas = [document.body.querySelector('a[data-fc-cta="1"]'), root.querySelector('.float-cta')];
          ctas.forEach(el => {
            if (!el) return;
            if (open) {
      ...
      navLinks.forEach(a => {
        a.addEventListener('click', (ev) => {
          ...
          if (!href.startsWith('#')) {
            // モバイルメニュー閉じるだけ
            if (menu && toggle) { menu.classList.remove('is-open'); toggle.classList.remove('is-open'); document.body.classList.remove('fc-menu-open'); }
            return;
          }
          ev.preventDefault();
