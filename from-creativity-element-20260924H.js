 ...
 *
 * 【2026-09-03V の変更】
 *   • メニューを開いてもフロートCTAが消えなかった不具合を修正。
 *     body直下に生成しているボタンには class が無く、
 *     data-fc-cta 属性だけだったためCSSが空振りしていた。
 *     セレクタに属性を追加し、生成時にクラスも付与。
 *     さらにトグル時にインラインでも隠す二重の保険を入れた。
 *
 * 【2026-09-03U の変更】
 *   • メニュー内の金色の区切りが太い棒になっていたのを修正。
 *     li 共通の padding と border-bottom が背景ごと描画されていた。
 *     幅44%・高さ1pxの細線に。
 *   • メニューの開始位置をヘッダーの実測高さ（--fc-navh）に連動。
 *     top:60px 固定だったため、先頭の Top がヘッダーに隠れていた。
 *   • メニューを開いている間はフロートCTA「無料相談する」を非表示に。
 *
 * 【2026-09-03T の変更】
 *   • スマホのメニューが最後まで出ない不具合を修正。
 *     項目が9個に増えて画面の高さを超えていたため、
 *     max-height と縦スクロールを付け、行間を圧縮した。
 *   • フロートCTA「無料相談する」が最下部の特商法ボタンに
 *     重なる問題を修正。判定を「タグラインを通過したら」から
 *     「タグラインが見えたら」に変更し、下余白も追加。
 *
 * 【2026-09-03S の変更】
 *   • Contact の文字がホバーするまで見えない不具合を修正。
 *     旧「nav.top ul li:last-child a{color:金}」が残っており、
 *     詳細度が .nav-contact より高いため、金背景に金文字が
 *     重なって消えていた。旧指定を撤去。
 *
 * 【2026-09-03R の変更】
 *   • ナビの並びを Legal → Contact に。Contact を金色のボタンに
 *   • ページ遷移リンク（Legal / Contact）を枠線付きにして、
 *     ページ内アンカー（Top/Service/…）と見た目で区別。間に区切り線
 *   • 最下部の「特定商取引法に基づく表記」を枠線付きボタンに拡大
 *
 * 【2026-09-03Q の変更】
 *   • ナビゲーションに「Legal」を追加（/legal へ）
 *   • タグラインの下に「特定商取引法に基づく表記」へのリンクを追加
 *
 * 【2026-08-13P の変更】
 ...
 *  Server URL: https://cdn.jsdelivr.net/gh/fromcreativity/from-creativity@main/from-creativity-element-20260924H.js
 *
...
(function(){
  ...
  const VERSION = '2026-09-24H-AWARDS';
...
nav.top .logo{
  ...
  padding:0 14px 0 6px;border-radius:30px;
  transition:background .4s;
}
nav.top .logo:hover{background:rgba(194,160,109,0.08)}
nav.top .logo svg{width:24px;height:44px;flex:0 0 24px;display:block;filter:drop-shadow(0 4px 10px rgba(194,160,109,0.25))}
nav.top .logo .logo-wordmark{display:flex;flex-direction:column;gap:4px;line-height:1.2}
nav.top .logo .logo-owner{font-family:var(--serif-jp);font-size:12px;font-weight:400;letter-spacing:0.24em;color:var(--gold);text-transform:none}
nav.top .logo .logo-text{
  font-family:Georgia,'Times New Roman',serif;text-transform:none;font-size:20px;letter-spacing:0.12em;
...
/* ページ遷移リンク（Legal / Contact）はアンカーと区別する */
nav.top ul li.nav-div{
  width:1px;height:14px;background:var(--gold-soft);
  margin:0 4px;flex:0 0 auto;
}
nav.top ul a.nav-page::after{display:none}
nav.top ul a.nav-page{
  color:rgba(245,241,232,0.72);
  border:1px solid rgba(194,160,109,0.28);
  padding:10px 18px;letter-spacing:0.22em;
}
nav.top ul a.nav-page:hover{
  color:var(--gold);border-color:var(--gold);
}
nav.top ul a.nav-contact{
  color:var(--ink);background:var(--gold);border-color:var(--gold);
  font-weight:500;letter-spacing:0.24em;
}
nav.top ul a.nav-contact::before{display:none}
nav.top ul a.nav-contact:hover{
  color:var(--ink);background:#F0DEB6;border-color:#F0DEB6;
}
/* ※ 旧「li:last-child」指定は撤去。
   .nav-contact より詳細度が高く、金文字が金背景に重なって
   ホバーするまで文字が見えない状態になっていた。 */
...
.yt-handle{font-family:var(--sans-jp);text-transform:none;font-size:11px;color:var(--gold);letter-spacing:0.3em}
...
.profile-role{font-family:var(--sans-jp);text-transform:none;font-size:11px;letter-spacing:0.4em;color:var(--ink);margin-bottom:32px;opacity:0.6}
...
.legal-link{margin-top:70px;padding-top:44px;border-top:1px solid rgba(194,160,109,0.22);max-width:560px;margin-left:auto;margin-right:auto}
.legal-link a{
  display:inline-flex;align-items:center;justify-content:center;gap:12px;
  font-family:var(--serif-jp);font-size:15px;font-weight:400;
  color:var(--gold);letter-spacing:0.12em;text-decoration:none;
  border:1px solid var(--gold-soft);padding:17px 34px;
  transition:color .45s,border-color .45s,background .45s,transform .45s;
}
.legal-link a::after{content:'→';font-family:var(--sans-jp);font-size:14px;transition:transform .45s}
.legal-link a:hover{
  color:var(--ink);background:var(--gold);border-color:var(--gold);
  transform:translateY(-3px);
}
.legal-link a:hover::after{transform:translateX(5px)}
...
/* メニューを開いている間はフロートCTAを隠す（項目に重なるため） */
body.fc-menu-open .float-cta,
body.fc-menu-open .float-cta-detached,
body.fc-menu-open a[data-fc-cta="1"]{
...
@media (max-width: 768px){
  ...
  nav.top{padding:14px 20px;background:rgba(10,9,7,0.85);backdrop-filter:blur(8px);mix-blend-mode:normal}
  nav.top .logo svg{width:24px;height:44px}
  nav.top .logo .logo-text{font-size:17px;letter-spacing:0.08em} nav.top .logo .logo-owner{font-size:11px;letter-spacing:0.2em}
  nav.top ul{display:flex;flex-direction:column;gap:0;position:fixed;top:var(--fc-navh,60px);left:0;right:0;background:#0A0907;padding:14px 20px 26px;border-top:1px solid var(--gold-soft);transform:translateY(-120%);transition:transform .4s cubic-bezier(.7,0,.3,1);pointer-events:none;max-height:calc(100vh - var(--fc-navh,60px));overflow-y:auto;-webkit-overflow-scrolling:touch;overscroll-behavior:contain}
  nav.top ul{max-height:calc(100dvh - var(--fc-navh,60px))}
  nav.top ul li.nav-div{
    width:44%;height:1px;margin:12px auto;padding:0 !important;
    background:rgba(194,160,109,0.35);border-bottom:none !important;
    flex:0 0 auto;
  }
  nav.top ul a.nav-page{display:block;text-align:center;margin:3px 0;padding:11px 14px}
  nav.top ul.is-open{transform:translateY(0);pointer-events:auto}
  nav.top ul li{border-bottom:1px solid rgba(245,241,232,0.1);padding:8px 0}
  nav.top ul li:last-child{border-bottom:none}
  nav.top ul a{display:block;font-size:13.5px;padding:8px 14px}
...
@media (max-width: 480px){
  ...
  .legal-link{margin-top:44px;padding-top:30px;max-width:100%;padding-left:18px;padding-right:18px}
  .legal-link a{font-size:13.5px;letter-spacing:0.08em;padding:15px 20px;width:100%;gap:8px}
...
@media (max-width:768px){
  ...
  .fc-app .tagline{padding-top:80px !important;padding-bottom:96px !important}
}
@media (max-width:480px){
  .fc-app section,
  .fc-app .awards,
...
@media(prefers-reduced-motion:reduce){
  .fc-app .reveal,.fc-app .hero h1 .char,.fc-app .hero .lead,.fc-app .hero-video{animation:none!important;transition:none!important;opacity:1!important;transform:none!important}
}
</style>`;
  const HTML = `
<main class="fc-app">
<nav class="top">
  <a href="#" class="logo" aria-label="From Creativity">
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 222 412" preserveAspectRatio="xMidYMid meet" fill="currentColor" color="#fff" aria-hidden="true">
  <g transform="translate(-107 -306)">
    <path fill-rule="evenodd" d="M108.5 306H328Q329 306 329 307V638Q329 639.5 327.7 638.9C276.9 625.39 241.56 583.8 223.5 533.6Q223.3 533 222.5 533H213.5Q212.7 533 212.5 533.6C194.44 583.8 159.09 625.39 108.3 638.9Q107.5 639.1 107.5 638V307Q107.5 306 108.5 306ZM137 334.5H167Q172.5 334.5 172.5 340V370Q172.5 375.5 167 375.5H137Q131.5 375.5 131.5 370V340Q131.5 334.5 137 334.5ZM137 404.5H167Q172.5 404.5 172.5 410V439Q172.5 444.5 167 444.5H137Q131.5 444.5 131.5 439V410Q131.5 404.5 137 404.5ZM137 474.5H167Q172.5 474.5 172.5 480V511Q172.5 516.5 167 516.5H137Q131.5 516.5 131.5 511V480Q131.5 474.5 137 474.5Z"/>
    <path d="M217.5 622.5C227.5 622.5 235.5 630.7 235.5 641V645.5C235.5 650 225.5 663.2 225.5 670.5V699.5C225.5 706 226.6 712 228.5 717.5H207C209.5 709.5 210 703.6 210 699.5L209.9 675C209.9 668.5 209.5 665.7 206.4 660C203.8 655 199.5 648.5 199.5 645.5V641C199.5 630.7 207.5 622.5 217.5 622.5Z"/>
  </g>
</svg>
    <span class="logo-wordmark"><span class="logo-text">From&nbsp;Creativity</span><span class="logo-owner">佐々木駿平</span></span>
  </a>
  ...
  <ul id="navMenu">
    ...
    <li><a href="https://www.from-creativity.com/legal" target="_top" class="nav-page">Legal</a></li>
  ...
  <div class="hero-grid">
    <div class="hero-text">
      ...
      <p class="lead">
        ...
        <span class="accent">世界に一つだけの</span>ドキュメンタリーを、元NHKディレクターの佐々木駿平が、丁寧な取材と言葉を生かす編集で残します。
      </p>
    </div>

    <div class="hero-video" id="concept-movie">
    ...
    <ul class="news-list">
      <li>
        <a href="#awards-list">
          ...
          <span class="news-title"><span class="news-tag">SELECTION</span>「澄江の空」が、フランス・パリで行われる人権映画祭で入選しました</span>
  ...
  <div class="portfolio-grid">
    ...
    <a class="work work--video span-3 reveal"
      ...
      <div class="work-info">
        ...
        <div class="work-year">2025 · 9 MIN｜東京・高田馬場</div>
      </div>
    </a>
  </div>
...
<section class="profile" id="about">
  <div class="profile-grid">
    ...
    <div class="profile-content reveal">
      ...
      <div class="profile-name">Shumpei Sasaki</div>
      <div class="profile-role">映像作家｜From Creativity 代表</div>
      <dl class="profile-meta">
        <dt>NAME</dt><dd><span>佐々木　駿平</span> / Shumpei Sasaki</dd>
        ...
        <dt>EMAIL</dt><dd><a href="mailto:sasaki@from-creativity.com">sasaki@from-creativity.com</a></dd>
      </dl>
      ...
      <div class="profile-awards" id="awards-list">
        <div class="profile-awards-h">受賞歴など ／ AWARDS &amp; SELECTIONS</div>
        <ul>
          <li>第 3 回ヤングリポーターコンペティション　毎日新聞社賞<span>2020</span></li>
          <li>東京ドキュメンタリー映画祭2020　入選<span>2020</span></li>
          <li>NHK 九州沖縄ブロック新人選奨　第 1 位<span>2021</span></li>
          ...
          <li>Artikas International Human Rights Short Film Festival 2026（フランス・パリ）　入選<span>2026</span></li>
        </ul>
        <div class="awards-foot">
          ※「ピアノが私にくれたもの」（毎日新聞社賞／東京ドキュメンタリー映画祭2020 入選）<br>
          ※「クマロク！」リポート企画（NHK 九州沖縄ブロック新人選奨）<br>
          ※短編ドキュメンタリー映画「A Man, A Hobby, A Whole Life」（Kolkata / NY Istanbul / Mannheim / Post-Cinema）<br>
          ※「澄江の空」（Artikas International Human Rights Short Film Festival 2026 入選）
        </div>
      </div>
      <div class="nhk-works">
  ...
  <div class="advisor reveal">
    ...
    <div class="advisor-content">
      ...
      <div class="advisor-name">Akihiro Nonaka</div>
      <div class="advisor-name-jp">野中　章弘</div>
...
<section class="message" id="message">
  <div class="content reveal">
    ...
    <div class="sig">
      <div>
        <div style="font-size:11px;letter-spacing:0.3em;color:var(--gold);font-family:Georgia,'Times New Roman',serif;text-transform:none;margin-bottom:6px">From Creativity&nbsp;·&nbsp;<span style="font-family:var(--serif-jp);font-size:12px;letter-spacing:0.18em;font-weight:500">代表</span></div>
...
`;
  ...
  class FromCreativityPageV4 extends HTMLElement {
    connectedCallback() {
      ...
      this.dataset.fcVersion = VERSION;
      this.fixWixWrapper();
      this.splitHeroTitle();
      ...
      requestAnimationFrame(() => requestAnimationFrame(() => {
        this.dataset.fcReady = 'true';
        document.dispatchEvent(new CustomEvent('fc:ready'));
      }));
    ...
    syncCtaVisibility() {
      ...
      const reached = tagline && tagline.getBoundingClientRect().top < window.innerHeight - 80;
      const hidden = document.body.classList.contains('fc-menu-open') || reached;
      const cta = document.body.querySelector('a[data-fc-cta="1"]') || this.querySelector('.float-cta');
      if (!cta) return;
      cta.style.setProperty('opacity', hidden ? '0' : '1', 'important');
      cta.style.setProperty('visibility', hidden ? 'hidden' : 'visible', 'important');
      cta.style.setProperty('pointer-events', hidden ? 'none' : 'auto', 'important');
      cta.style.transform = hidden ? 'translateY(20px)' : '';
    }
    hideCtaOnTagline() {
      this._fcCtaUpdate = () => this.syncCtaVisibility();
      window.addEventListener('scroll', this._fcCtaUpdate, { passive: true });
      window.addEventListener('resize', this._fcCtaUpdate, { passive: true });
      this.syncCtaVisibility();
    }
    /* 「無料相談する」ボタンを確実に表示。
    ...
    ensureFloatCta() {
      ...
      try {
        ...
        const styleStr = (
          ...
          'z-index:2147483646;background:#C2A06D;color:#0A0907;' +
    ...
    disconnectedCallback() {
      window.removeEventListener('scroll', this._fcCtaUpdate);
      window.removeEventListener('resize', this._fcCtaUpdate);
        ...
        const syncNavHeight = () => {
          try {
            ...
            if (nav) {
              ...
              if (h > 20) document.documentElement.style.setProperty('--fc-navh', h + 'px');
            }
          } catch (e) { /* noop */ }
        };
        syncNavHeight();
        window.addEventListener('resize', syncNavHeight, { passive: true });
        [300, 1200, 3000].forEach(t => setTimeout(syncNavHeight, t));

        toggle.addEventListener('click', () => {
          syncNavHeight();
          const open = menu.classList.toggle('is-open');
          toggle.classList.toggle('is-open', open);
          document.body.classList.toggle('fc-menu-open', open);
          toggle.setAttribute('aria-expanded', String(open));
          toggle.setAttribute('aria-label', open ? 'メニューを閉じる' : 'メニューを開く');
          root.syncCtaVisibility();
        });
      }
      // メニューリンクのスムーズスクロール
