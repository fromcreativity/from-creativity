...
.fc-app .hero.hero-cinematic{
  ...
  display:block;
  width:100% !important;
  min-height:clamp(780px,98svh,980px);
  padding:0 !important;
  margin:0 !important;
  overflow:hidden;
  background:#000;
...
.fc-app .hero.hero-cinematic::after{
  ...
  z-index:1;
  pointer-events:none;
  background:linear-gradient(180deg,rgba(0,0,0,.36),transparent 20%,transparent 80%,rgba(0,0,0,.52));
}
.fc-app .hero-cinematic .hero-grid{
  ...
  min-height:clamp(780px,98svh,980px);
  margin:0 auto;
  padding:clamp(112px,12vh,148px) 0 72px clamp(42px,4.3vw,82px);
}
...
.fc-app .hero-title-break{display:block}
...
.fc-app .hero-cinematic .hero-media::after{
  ...
  box-shadow:none;
  background:
    linear-gradient(90deg,#000 0%,rgba(0,0,0,.50) 3%,transparent 10%,transparent 92%,rgba(0,0,0,.42) 97%,#000 100%),
    linear-gradient(180deg,#000 0%,rgba(0,0,0,.26) 4%,transparent 11%,transparent 89%,rgba(0,0,0,.30) 96%,#000 100%);
}
...
.fc-app .hero-seek::-webkit-slider-runnable-track{
  width:100%;
  height:3px;
  border:0;
  background:linear-gradient(90deg,var(--gold) 0%,#ead2aa var(--seek-progress),rgba(245,241,232,.18) var(--seek-progress),rgba(245,241,232,.18) 100%);
}
.fc-app .hero-seek::-webkit-slider-thumb{
  width:15px;
  height:15px;
  margin-top:-6px;
...
.fc-app .hero-seek::-moz-range-track{
  width:100%;
  height:3px;
  border:0;
  background:rgba(245,241,232,.18);
}
.fc-app .hero-seek::-moz-range-progress{height:3px;background:linear-gradient(90deg,var(--gold),#ead2aa)}
.fc-app .hero-seek::-moz-range-thumb{
  width:13px;
  height:13px;
  border:2px solid #000;
  border-radius:50%;
...
.fc-app .hero-seek:focus-visible{outline:2px solid var(--gold);outline-offset:-2px}
...
.fc-app .hero-visual:fullscreen .hero-media::after,
.fc-app .hero-visual:-webkit-full-screen .hero-media::after{background:none;box-shadow:none}
...
@media (max-width:1100px) and (min-width:961px){
  ...
  .fc-app .hero-cinematic .hero-text{width:100%}
  .fc-app .hero-cinematic h1{
    width:100%;
    font-size:58px;
    line-height:1.18;
  }
  .fc-app .hero-title-line--long{font-size:.96em}
  .fc-app .hero-cinematic h1 em{font-size:16px}
  .fc-app .hero-cinematic .lead{max-width:510px;font-size:16px}
  .fc-app .hero-visual{
    position:relative;
    top:auto;
    right:auto;
...
@media (max-width:960px){
  ...
  .fc-app .hero-title-break{display:none}
  .fc-app .hero-cinematic h1 em{
    margin-top:16px;
    font-size:12px;
    line-height:1.65;
  ...
  .fc-app .hero-cinematic .hero-media::after{
    ...
    box-shadow:none;
    background:
      linear-gradient(90deg,rgba(0,0,0,.72) 0%,transparent 6%,transparent 94%,rgba(0,0,0,.72) 100%),
      linear-gradient(180deg,rgba(0,0,0,.78) 0%,transparent 9%,transparent 91%,rgba(0,0,0,.78) 100%);
  }
  .fc-app .hero-reel-ui{
    display:block;
    margin-top:0;
...
@media (max-width:380px){
  ...
  .fc-app .hero-cinematic h1{font-size:27px}
  ...
  <div class="hero-grid">
    <div class="hero-text">
      <h1>
        ...
        <span class="hero-title-line hero-title-line--long">語るべき物語を<br class="hero-title-break">持っている。</span>
    ...
    <div class="hero-visual">
      <div class="hero-media">
        ...
        <input class="hero-seek" type="range" min="0" max="1000" step="1" value="0"
               aria-label="コンセプトムービーの再生位置" aria-valuetext="0:00">
      </div>
...
`;
  ...
  class FromCreativityPageV4 extends HTMLElement {
    ...
    fixWixWrapper() {
      try {
        /* Wixが保存した巨大な --custom-element-height を子要素へmin-heightとして
           適用する場合がある。実コンテンツの高さへ戻し、ページ末尾の空白を防ぐ。 */
        ...
        while (el && el !== document.body && el !== document.documentElement) {
          ...
          if (isWixSection) {
            el.style.setProperty('--custom-element-height', 'auto');
    ...
    */
    initHeroVideo() {
      try {
        ...
        const seek = this.querySelector('.hero-seek');
        ...
        const updateProgress = () => {
          ...
          const progressPercent = (ratio * 100).toFixed(3) + '%';
          seek.value = String(Math.round(ratio * 1000));
          seek.style.setProperty('--seek-progress', progressPercent);
          seek.setAttribute('aria-valuetext', formatTime(video.currentTime) + ' / ' + formatTime(duration));
        };
        ...
        if (seek) {
          const moveToSelectedTime = () => {
            const duration = Number.isFinite(video.duration) && video.duration > 0 ? video.duration : 0;
            if (!duration) return;
            const ratio = Math.min(1, Math.max(0, Number(seek.value) / 1000));
            try { video.currentTime = ratio * duration; } catch (e) { /* metadata待ち */ }
            updateProgress();
          };
          seek.addEventListener('input', moveToSelectedTime);
          seek.addEventListener('change', () => {
            moveToSelectedTime();
            announce(formatTime(video.currentTime) + 'へ移動しました');
          });
        }
